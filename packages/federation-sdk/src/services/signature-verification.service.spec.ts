...
describe('SignatureVerificationService', () => {
    ...
    describe('verifySignature', () => {
        ...
        it('should verify signature using provided public key function', async () => {
            const verifyMock = spyOn(nacl.sign.detached, 'verify').mockReturnValue(true);

            const getPublicKey = async (_origin: string, _keyId: string) => mockPublicKey;

            const result = await service.verifySignature(mockEvent, mockOriginServer, getPublicKey);
        ...
        it('should return false when signature verification fails', async () => {
            const verifyMock = spyOn(nacl.sign.detached, 'verify').mockReturnValue(false);
        ...
        it('should exclude signatures and unsigned fields when verifying', async () => {
            ...
            const result = await service.verifySignature(eventWithUnsigned, mockOriginServer);
