...
function createMockEvent(eventId: string, roomId: string, origin: string): EventBase {
    return {
        event_id: eventId,
        room_id: roomId,
        type: 'test.event',
...
describe('MissingEventListener', () => {
    ...
    beforeEach(() => {
        ...
        mockEventService = {
            ...
            checkIfEventsExists: mock(() => Promise.resolve({ missing: [], found: [] })),
        ...
        mockEventFetcherService = {
            fetchEventsByIds: mock(() => Promise.resolve({ events: [], missingEventIds: [] }))
        };
    ...
    test('handleQueueItem should exit early if event already exists', async () => {
        mockEventService.checkIfEventExistsIncludingStaged.mockReturnValue(Promise.resolve(true));
        ...
        expect(mockEventService.checkIfEventExistsIncludingStaged).toHaveBeenCalledWith('existing-event-id');
        expect(mockEventService.removeDependencyFromStagedEvents).toHaveBeenCalledWith('existing-event-id');
    ...
    test('handleQueueItem should process fetched events with no missing dependencies', async () => {
        ...
        mockEventService.checkIfEventExistsIncludingStaged.mockReturnValue(Promise.resolve(false));
        mockEventFetcherService.fetchEventsByIds.mockReturnValue(Promise.resolve({
            events: [{ eventId, event: mockEvent }],
            missingEventIds: []
        }));
        mockEventService.checkIfEventsExists.mockReturnValue(Promise.resolve({
            missing: [],
            found: ['auth1', 'prev1']
        }));
        mockEventService.findStagedEvents.mockReturnValue(Promise.resolve([{
            _id: eventId,
            event: mockEvent,
            origin,
        ...
        expect(mockEventService.checkIfEventExistsIncludingStaged).toHaveBeenCalledWith(eventId);
        expect(mockEventFetcherService.fetchEventsByIds).toHaveBeenCalledWith([eventId], roomId, origin);
        expect(mockEventService.checkIfEventsExists).toHaveBeenCalledWith(['auth1', 'prev1']);
        expect(mockEventService.storeEventAsStaged).toHaveBeenCalledWith(expect.objectContaining({
            _id: eventId,
            event: mockEvent,
            origin,
            missing_dependencies: []
        }));
        expect(mockEventService.removeDependencyFromStagedEvents).toHaveBeenCalledWith(eventId);
    ...
    test('handleQueueItem should process fetched events with missing dependencies', async () => {
        ...
        expect(mockEventService.checkIfEventExistsIncludingStaged).toHaveBeenCalledWith(eventId);
        expect(mockEventFetcherService.fetchEventsByIds).toHaveBeenCalledWith([eventId], roomId, origin);
        expect(mockEventService.checkIfEventsExists).toHaveBeenCalledWith(['auth1', 'auth2', 'prev1']);
        expect(mockEventService.storeEventAsStaged).toHaveBeenCalledWith(expect.objectContaining({
            _id: eventId,
            event: mockEvent,
            origin,
    ...
    test('handleQueueItem should handle errors during fetch', async () => {
        ...
        expect(mockEventService.checkIfEventExistsIncludingStaged).toHaveBeenCalled();
