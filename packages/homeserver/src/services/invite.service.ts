...
export class InviteService {
	...
	async processInvite(
		...
		logger.info(
			`Processing invite for room ${roomId} from ${event.sender} to ${event.state_key} ${eventId}`,
		);
	...
	private async handleInviteProcessing(event: any): Promise<void> {
		try {
			logger.info(
				`Starting invite processing for ${event.state_key} in room ${event.room_id}`,
			);

			const signingKey = await this.configService.getSigningKey();
			logger.info(
				`Loaded signing key for server ${this.configService.getServerConfig().name}`,
			);
			...
			logger.info(
				`Making join request to ${event.origin} for room ${event.room_id}`,
			);
			...
			logger.info(
				`Sending join event to ${event.origin} for room ${event.room_id}`,
			);
			...
			const createEvent = responseBody.state.find(
				(e: any) => e.type === "m.room.create",
			);
			if (!createEvent) {
				throw new MatrixError(
					"400",
					"Invalid response: missing m.room.create event",
				);
			}
			...
			logger.info(
				`Processing auth chain with ${responseBody.auth_chain.length} events and state with ${responseBody.state.length} events`,
			);
			...
			for await (const [eventId, pduEvent] of [
				...
				try {
					...
					logger.error(
						`Error checking signature for event ${eventId}: ${e.message}`,
					);
					invalidCount++;
				}
			}
			...
			logger.info(
				`Validated PDUs: ${validCount} valid, ${invalidCount} invalid`,
			);
			...
			logger.info(
				`Upserting room ${signedCreateEvent[1].room_id} with ${validPDUs.size} events`,
			);
			...
			logger.info(
				`Successfully processed invite for ${event.state_key} in room ${event.room_id}`,
			);
		} catch (error: any) {
			logger.error(
