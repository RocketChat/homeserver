...
export class EventService {
  ...
  async processIncomingPDUs(events: roomV10Type[]) {
    ...
    const keysRequired = eventsWithIds.reduce(
      (accum, { event: { signatures } }) => {
        // biome-ignore lint/complexity/noForEach: <explanation>
        Object.keys(Object.values(signatures)).forEach((keyId) =>
          accum.add(keyId)
        );
        return accum;
      },
    ...
    const keys = await this.keyService.getValidVerifyKeys(events[0].origin, [
      ...keysRequired.values(),
    ]);
    // use the keys above to validatye
  ...
  private async validateSignaturesAndHashes(
    ...
    const getPublicKeyFromServer = async (origin: string, keyId: string) => {
      const key = await this.keyRepository.findKey(
        origin,
        keyId,
