...
export class KeyService {
		...
		return keys.every((key) => {
			if (validUntil) {
				if (key.valid_until_ts < validUntil) {
					logger.log(
						`Key for ${serverName} is expired, ${key.valid_until_ts} < ${validUntil}, refetching keys`,
					);
					return true;
				}
	...
	async fetchKeys(
	...
	): Promise<WithId<ServerKeyDocument>[]> {
		// 1. check db
		const keysCursor = await this.keyRepository.findKeys(
			serverName,
			keyId,
			validUntil,
		);
		...
		if (!this.shouldRefetchKeys(serverName, keys, validUntil)) {
			logger.log(
				`Keys for ${serverName} are not expired, returning cached keys`,
			);
			return keys;
		}
