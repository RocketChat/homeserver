...
export class ServerController {
  ...
  private async handleQuery(body: V2KeyQueryBody): Promise<V2KeyQueryResponse> {
    const keys: Omit<KeyV2ServerResponse, "signatures">[] = [];
    ...
    for (const [serverName, request] of Object.entries(body.server_keys)) {
      ...
      if (_keys.length === 0) {
        logger.info(
          `no explicit key requested for ${serverName}, fetching all keys`
        );
        // no key and criteria, make sure resulting keys are not expired, compare against current time
        const key = await this.keyService.fetchAndStoreKey(serverName, {
          validUntil: Date.now(),
        });
        if (key) {
          keys.push(
            this.keyService.convertKeyToKeyV2ServerResponse({
              serverName,
              keys: key,
            })
          );
      ...
      for (const keyId of _keys) {
        ...
        logger.info(
          `fetching keys for ${serverName}, ${keyId}, ${criteria.minimum_valid_until_ts}`
        );
        const key = await this.keyService.fetchAndStoreKey(serverName, {
          keyId,
          validUntil: criteria.minimum_valid_until_ts,
        });
        if (key) {
          keys.push(
            this.keyService.convertKeyToKeyV2ServerResponse({
      ...
      server_keys: await Promise.all(
        keys.map(async (key) => {
          const signed = await signJson(
            key,
            (
              await this.configService.getSigningKey()
            )[0],
          ...
          return {
            ...
            signatures: {
              ...signed.signatures,
              // spec does not require us to sign the keys
              // ...signatures,
            },
          };
        })
