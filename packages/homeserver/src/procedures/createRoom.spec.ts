...
test("createRoom auth_events and prev_events chain correctly", async () => {
	...
	const [createEvent, memberEvent, powerLevelsEvent, joinRulesEvent, historyVisibilityEvent, guestAccessEvent] = events;
	expect(memberEvent.event.prev_events).toEqual([createEvent._id]);
	expect(powerLevelsEvent.event.auth_events).toEqual([createEvent._id, memberEvent._id]);
	expect(powerLevelsEvent.event.prev_events).toEqual([memberEvent._id]);
	expect(joinRulesEvent.event.auth_events).toEqual([createEvent._id, memberEvent._id, powerLevelsEvent._id]);
	expect(joinRulesEvent.event.prev_events).toEqual([powerLevelsEvent._id]);
	expect(historyVisibilityEvent.event.auth_events).toEqual([createEvent._id, memberEvent._id, powerLevelsEvent._id]);
	expect(historyVisibilityEvent.event.prev_events).toEqual([joinRulesEvent._id]);
	expect(guestAccessEvent.event.auth_events).toEqual([createEvent._id, memberEvent._id, powerLevelsEvent._id]);
	expect(guestAccessEvent.event.prev_events).toEqual([historyVisibilityEvent._id]);
});
