...
export async function validateMemberEvent(
  ...
  try {
    ...
    if (rawEvent.state_key === undefined || rawEvent.state_key === '') {
      console.error(`Member event ${eventId} has invalid state_key: '${rawEvent.state_key}'`);
    ...
    if (!content.membership) {
      console.error(`Member event ${eventId} is missing required membership field`);
    ...
    if (!Object.values(Membership).includes(content.membership)) {
      console.error(`Member event ${eventId} has invalid membership value: ${content.membership}`);
    ...
    if (!auth_event_objects || auth_event_objects.length === 0) {
      console.error(`Member event ${eventId} is missing required auth events`);
    ...
    if (!createEvent) {
      console.error(`Member event ${eventId} missing required m.room.create event`);
    ...
    if (content.membership === Membership.JOIN) {
      ...
      if (!joinRules) {
        console.warn(`Join event ${eventId} missing join_rules event`);
        // Don't fail but warn
      } 
        ...
        if (rawEvent.sender !== createEvent.content?.creator) {
          ...
          if (!previousMembership || previousMembership.content?.membership !== Membership.INVITE) {
            console.error(`Join event ${eventId} for invite-only room without proper invitation`);
      ...
      if (!powerLevels) {
        console.warn(`Invite event ${eventId} missing power_levels event`);
        // Don't fail but warn
      } 
      else {
        ...
        if (userPowerLevel < invitePowerLevel) {
          console.error(`Invite event ${eventId} sender ${rawEvent.sender} has insufficient power: ${userPowerLevel} < ${invitePowerLevel}`);
    ...
    if (content.membership === Membership.INVITE) {
      ...
      if (content.third_party_invite) {
        ...
        if (typeof content.third_party_invite !== 'object') {
          console.error(`Member event ${eventId} has invalid third_party_invite (not an object)`);
        ...
        if (!content.third_party_invite.signed || typeof content.third_party_invite.signed !== 'object') {
          console.error(`Member event ${eventId} has invalid third_party_invite.signed (missing or not an object)`);
        ...
        if (!signed.mxid || !signed.token || !signed.signatures) {
          console.error(`Member event ${eventId} has invalid third_party_invite.signed (missing required fields)`);
        ...
        if (signed.mxid !== rawEvent.state_key) {
          console.error(`Member event ${eventId} has third_party_invite.signed.mxid (${signed.mxid}) that doesn't match state_key (${rawEvent.state_key})`);
    ...
    if (content.join_authorised_via_users_server) {
      if (typeof content.join_authorised_via_users_server !== 'string') {
        console.error(`Member event ${eventId} has invalid join_authorised_via_users_server (not a string)`);
      ...
      if (!content.join_authorised_via_users_server.includes('.')) {
        console.warn(`Member event ${eventId} has suspicious join_authorised_via_users_server value: ${content.join_authorised_via_users_server}`);
        // We don't fail on this, just warn
      }
    }
    
    console.debug(`Member event ${eventId} passed validation`);
    return success(event);
    
  } catch (error: any) {
    console.error(`Error validating member event ${eventId}: ${error.message || String(error)}`);
