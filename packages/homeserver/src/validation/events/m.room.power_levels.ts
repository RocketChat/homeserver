...
export async function validatePowerLevels(
  ...
  try {
    ...
    if (rawEvent.state_key !== '') {
      console.error(`Power levels event ${eventId} has invalid state_key: '${rawEvent.state_key}'`);
    ...
    if (!auth_event_objects || auth_event_objects.length === 0) {
      console.error(`Power levels event ${eventId} is missing required auth events`);
    ...
    if (!createEvent) {
      console.error(`Power levels event ${eventId} missing required m.room.create event`);
    ...
    if (!senderMembership) {
      console.error(`Power levels event ${eventId} missing sender's membership event`);
    ...
    if (senderMembership.content?.membership !== 'join') {
      console.error(`Power levels event ${eventId} sender is not joined to the room`);
    ...
    for (const field of numericalFields) {
      if (content[field] !== undefined && 
        ...
        console.error(`Power levels event ${eventId} has invalid ${field}: ${content[field]}`);
    ...
    if (content.events && typeof content.events !== 'object') {
      console.error(`Power levels event ${eventId} has invalid events field (not an object)`);
      return failure('M_INVALID_PARAM', 'events must be an object');
    }
    
    if (content.users && typeof content.users !== 'object') {
      console.error(`Power levels event ${eventId} has invalid users field (not an object)`);
    ...
    if (content.events) {
      for (const [eventType, powerLevel] of Object.entries(content.events)) {
        if (typeof powerLevel !== 'number' || !Number.isInteger(powerLevel)) {
          console.error(`Power levels event ${eventId} has invalid power level for ${eventType}: ${powerLevel}`);
    ...
    if (content.users) {
      for (const [userId, powerLevel] of Object.entries(content.users)) {
        if (typeof powerLevel !== 'number' || !Number.isInteger(powerLevel)) {
          console.error(`Power levels event ${eventId} has invalid power level for ${userId}: ${powerLevel}`);
    ...
    if (previousPowerLevels) {
      ...
      if (!isCreator && userPowerLevel < eventPowerLevel) {
        console.error(`Power levels event ${eventId} sender has insufficient power: ${userPowerLevel} < ${eventPowerLevel}`);
      ...
      if (content.users) {
        for (const [userId, newPowerLevel] of Object.entries(content.users)) {
          ...
          if (userId !== rawEvent.sender && // users can demote themselves
            ...
            console.error(`Power levels event ${eventId} tries to change power of ${userId} from ${currentPowerLevel} to ${newPowerLevel}, but sender only has ${userPowerLevel}`);
    ...
    console.debug(`Power levels event ${eventId} passed validation`);
    return success(event);
    
  } catch (error: any) {
    console.error(`Error validating power levels event ${eventId}: ${error.message || String(error)}`);
