...
export class KeyRepository {
	...
	async storeKey(key: ServerKey): Promise<void> {
		...
		await collection.insertOne({
			...key, _createdAt: new Date(),
		});
	}

	async findKey(serverName: string, keyId: string, validUntil: number): Promise<WithId<ServerKeyDocument> | null> {
		const collection = await this.getCollection();
	...
	async findKeys(serverName: string, keyId?: string, validUntil?: number): Promise<FindCursor<WithId<ServerKeyDocument>>> {
		const collection = await this.getCollection();
