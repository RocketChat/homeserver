...
import { loadEd25519SignerFromSeed, loadEd25519VerifierFromPublicKey, signJson, verifyJsonSignature } from './keys';
...
describe('Signing and verifying payloads', async () => {
	const seedFileContent = 'ed25519 a_XRhW YjbSyfqQeGto+OFswt+XwtJUUooHXH5w+czSgawN63U';
	...
	it('should verify a valid signature', async () => {
		const content = 'ed25519 a_yNbw tBD7FfjyBHgT4TwhwzvyS9Dq2Z9ck38RRQKaZ6Sz2z8';
	...
	it('should throw an error if the signature is invalid', async () => {
		...
		const verifier = await loadEd25519VerifierFromPublicKey(fromBase64ToBytes(diffPublicKeyContent), '0');

		expect(verifyJsonSignature({}, signature, verifier)).rejects.toThrowError('Invalid signature');
	});
});
...
describe('Canonical json serialization', () => {
	...
	const testCases: Array<{ input: Record<string, unknown>; expected: string }> = [
		...
		// Large nested structure with mixed types
		{
			...
			expected: '{"a":"test","m":false,"n":null,"z":[5,{"a":{"b":"c"},"x":10,"y":[1,2,3]}]}',
		},
