...
import { encodeCanonicalJson, EncryptionValidAlgorithm, signJson, toBinaryData, verifySignature } from ".";
...
describe("signJson", () => {
	it("should sign a json object", async () => {
		const json = {
			...
			content: {"edus":[{"content":{"push":[{"last_active_ago":45931,"presence":"offline","user_id":"@debdut:syn1.tunnel.dev.rocket.chat"}]},"edu_type":"m.presence"}],"origin":"syn1.tunnel.dev.rocket.chat","origin_server_ts":1743490730808,"pdus":[]},
			// signatures: {
	...
	it("should verify a signature", async () => {
		...
		const keyv2serverresponsefromorigin = {"old_verify_keys":{},"server_name":"syn1.tunnel.dev.rocket.chat","signatures":{"syn1.tunnel.dev.rocket.chat":{"ed25519:a_FAET":"MZF+8pncxhUNp7JzdSTIqriaANQ4QTYTe1AIqBNAtVhWcKz1Mc/6nzkP3/1HXZHAzCLYrmuFnTGb874XT4TJDg"}},"valid_until_ts":1747753307525,"verify_keys":{"ed25519:a_FAET":{"key":"kryovKVnhHESOdWuZ05ViNotRMVdEh/mG2yJ0npLzEo"}}}
		
		const verifyKey = keyv2serverresponsefromorigin.verify_keys["ed25519:a_FAET"].key;
		
		const content = encodeCanonicalJson(json);
		
		await verifySignature(content, new Uint8Array(Buffer.from(signature, "base64")), new Uint8Array(Buffer.from(verifyKey, "base64")), {
